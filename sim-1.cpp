...
bool carArrival() //this is my global bool for coinflip
{
  ...
  coinFlip = rand() % 4 + 1; //random gen number from 1 to 4 
...
private:
  ...
  const int  maxCycles = 333333; //in 1 million seconds, I've calculated that the lights will cycle this number of times (1mil/3)
  ...
  for (i = 0; i < 3; i++)
	{
	  ...
	  for (i = 0; i < 3; i++) //looping through my lanes 
	    {
...
int main()
{
  ...
  //initiate each road into the array[4]
   ...
   lanes[bottom] = 50;
   lanes[left] = 60;
   //There are 0 cars at each road to start this simulation.This is more my preference to have it start at 0 or 1 than anything.
